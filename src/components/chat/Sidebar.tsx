...
export function Sidebar() {
  ...
  return (
    <div className="flex h-full flex-col bg-gray-50/80 dark:bg-gray-900/95">
        ...
        <div className="flex items-center gap-1">
          {tab === "groups" && (
            <button
              ...
              className="rounded-xl p-2.5 text-gray-500 transition-colors active:bg-gray-200 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
              title="New group"
            >
      ...
      {/* Tabs */}
      <div className="mx-4 mb-3 flex rounded-xl bg-gray-100 p-1 dark:bg-gray-800">
        <button
          onClick={() => setTab("dms")}
        ...
        <div className="px-4 pb-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-gray-700 placeholder-gray-400 shadow-sm outline-none transition-shadow focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:placeholder-gray-500"
            />
          </div>
        </div>
      )}
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl px-3 py-3.5 transition-all active:scale-[0.98] ${
                      isSelected ? "bg-blue-50 shadow-sm dark:bg-blue-500/10" : "hover:bg-white dark:hover:bg-gray-800/60"
                    }`}
                  >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 pb-4">
          {groups?.map((group) => {
            ...
            return (
              ...
              >
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 text-sm font-bold text-white">
          ...
          {groups?.length === 0 && (
            <div className="px-4 py-12 text-center">
              <p className="mb-2 text-sm font-medium text-gray-500 dark:text-gray-400">No groups yet</p>
              <button
                onClick={() => setShowCreateGroup(true)}
      ...
      {session?.user && (
        <div className="border-t border-gray-200/80 bg-white/60 px-5 py-4 glass dark:border-gray-700/50 dark:bg-gray-900/60">
          <div className="flex items-center gap-3">
            <Avatar name={session.user.name ?? "User"} image={session.user.image} size="sm" online />
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-gray-900 dark:text-white">{session.user.name}</p>
              <p className="truncate text-xs text-gray-500 dark:text-gray-400">{session.user.email}</p>
            </div>
            <button
              onClick={() => signOut()}
              className="rounded-xl p-2.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300"
              title="Sign out"
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
